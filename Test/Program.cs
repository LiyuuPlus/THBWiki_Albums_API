...
namespace Test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var res4 = query.GetAlbum(album);
                if(res4.ErrorCode==QueryError.NO_ERROR)
                {
                    Console.WriteLine($"专辑获取测试：\n关键词：{album}");
                    Console.WriteLine($"专辑名：{res4.alname}");
                    Console.WriteLine($"制作社团：{res4.circle}");
                    Console.WriteLine($"发售年份：{res4.year}");
                    Console.WriteLine($"发售时间：{res4.date}");
                    Console.WriteLine($"封面角色：{res4.coverchar}");
